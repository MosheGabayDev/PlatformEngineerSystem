...
@blueprint.route('/sync-aws-servers', methods=['POST'])
@login_required
def sync_aws_servers():
    ...
    try:
        ...
        if success:
            return jsonify({'message': 'Successfully synced servers from AWS'}), 200
        else:
            return jsonify({'error': 'Failed to sync servers from AWS'}), 500
    except Exception as e:
